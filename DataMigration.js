...
const DataMigration_Network_refund = async function () {
    try {
        ...
        const bulkOperations = data.map((item) => {
            const { _id, internal_status = null, user_id, transaction_number, ...rest } = item;
            let sender = `IXFI ${transaction_number}`
            if (internal_status === "network_fee_refunded") sender = "@IXFI NETWORK FEE REFUND"
        ...
        const result = await db.collection(kytCollectionName).bulkWrite(bulkOperations);
...
const DataMigration_Transaction_Id = async function () {
    try {
        ...
        const bulkOperations = data.map((item) => {
            const { _id, user_id, transaction_type, transaction_number, systemTags, ...rest } = item;
...
const AddReceiverAddress = async function () {
    try {
        console.log(`Starting data mapping sender to receiver for platform: ${platformName}`);
        ...
        const bulkOperations = data.map((item) => {
            const { _id, user_id, transaction_number, systemTags, transaction_id, sender_doc = null, transaction_type, ...rest } = item;

            let transformedItem = { ...rest, _id, transaction_id, systemTags, transaction_type, user_id, transaction_number, platform: platformName };
...
const AddSenderReceiverDetails = async function () {
    try {
        ...
        const bulkOperations = data.map((item) => {
            const { _id, receiverEmail='',receiverName='',senderEmail='',senderName='', ...rest } = item;


            let transformedItem = { _id, senderEmail,
                senderName,
                receiverEmail,
                receiverName, ...rest };
            return {
        ...
        console.log(
            `data mapping sender and receiver from users schema`
        );
        console.log(
            `${result.matchedCount} documents matched, ${result.modifiedCount} updated, and ${result.upsertedCount} inserted into ${kytCollectionName}.`
        );
    } catch (error) {
        console.error("Error during data mapping sender and receiver from users schema:", error.message);
        console.error(error);
    }
}
...
const AddSenderAddress = async function () {
    try {
        console.log(`Starting data mapping receiver to sender for platform: ${platformName}`);
        ...
        const bulkOperations = data.map((item) => {
            const { _id, user_id, transaction_number, transaction_id, receiver_doc = null, transaction_type, ...rest } = item;

            let transformedItem = { ...rest, _id, user_id, transaction_id, transaction_type, transaction_number, platform: platformName };
...
const GropingTransaction = async function () {
    try {
        ...
        const [sendRecTrans, rewardsTrans, otherTrans] = await Promise.all([
            db.collection(kytCollectionName).aggregate(groupBasedOnTranSendAndRec).toArray(),
            db.collection(kytCollectionName).aggregate(groupBasedOnRewardsRemoveDuplicate).toArray(),
            db.collection(kytCollectionName).aggregate(groupOtherTransactionType).toArray(),
        ]);
        ...
        // Execute bulkWrite
        const result = await db.collection(groupingKytCOllectionName).bulkWrite(bulkOperations);
...
const GropingTransactionBasedOnDate = async function () {
    try {
        ...
        // Fetch grouped data
        const records = await db.collection(groupingKytCOllectionName).aggregate(groupTransactionFrequency).toArray()
        ...
        // Execute bulkWrite
        const result = await db.collection(groupingByDateCollectionName).bulkWrite(bulkOperations);
